  ...
  public async generateTools(context: AdapterGenerationContext): Promise<FridayyToolDefinition[]> {
    ...
    tools.push({
      ...
      metadata: {
        createdAt: new Date().toISOString(),
        autoGenerated: true,
        // The method/path used at execution time are supplied by the caller at
        // call-time, not fixed here. The tool handler re-classifies each call
        // against the actual method/path before executing it (see ToolHandler),
        // so a DELETE (or other destructive) call is still blocked/pending review
        // rather than inheriting this tool's static WRITE/APPROVED classification.
